...
public class DatabaseHandlerExpense extends SQLiteOpenHelper {
    ...
    public List<expenseModel> searchExpenses(String query) {
        List<expenseModel> searchResults = new ArrayList<>();
        SQLiteDatabase db = this.getReadableDatabase();

        String searchQuery = "SELECT * FROM " + TABLE_NAME +
                " WHERE " + COL3 + " LIKE '%" + query + "%'" +
                " OR " + COL3 + " LIKE '%" + query + "%'";
        ...
        if (cursor.moveToFirst()) {
            do {
                String id = cursor.getString(cursor.getColumnIndexOrThrow(COL1));
                String amount = cursor.getString(cursor.getColumnIndexOrThrow(COL2));
                String type = cursor.getString(cursor.getColumnIndexOrThrow(COL3));
                String note = cursor.getString(cursor.getColumnIndexOrThrow(COL4));
                String date = cursor.getString(cursor.getColumnIndexOrThrow(COL5));
